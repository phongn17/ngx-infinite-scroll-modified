...
export function createScroller(config: Models.IScroller) {
  ...
  const resolver = createResolver({ axis: new AxisResolver(!config.horizontal), windowElement: resolveContainerElement(scrollContainer, scrollWindow, element, fromRoot) });
  const { totalToScroll: startWithTotal } = calculatePoints(element, resolver);
  const scrollState: Models.IScrollState = { lastScrollPosition: 0, lastTotalToScroll: 0, totalToScroll: startWithTotal, triggered: { down: 0, up: 0 } };
  const options: Models.IScrollRegisterConfig = { container: resolver.container, throttle: config.throttle };
  const distance = { up: config.upDistance, down: config.downDistance };
  return attachScrollEvent(options).pipe(
    ...
    tap(({ stats, scrollDown }: Models.IScrollParams) => ScrollResolver.updateScrollState(scrollState, stats.scrolled, stats.totalToScroll)),
    filter(({ fire, scrollDown, stats: { totalToScroll } }: Models.IScrollParams) => shouldTriggerEvents(config.alwaysCallback, fire, ScrollResolver.isTriggeredScroll(
            totalToScroll,
            scrollState,
            scrollDown))),
    tap(({ scrollDown, stats: { totalToScroll } }: Models.IScrollParams) => { ScrollResolver.updateTriggeredFlag(totalToScroll, scrollState, true, scrollDown); }),
